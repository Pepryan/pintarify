        ...
        <div class="col-md-12">
            @can('test_result_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.test-results.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $testResult->id }}">
                                        ...
                                        <td>
                                            @can('test_result_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.test-results.show', $testResult->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('test_result_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.test-results.edit', $testResult->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('test_result_delete')
                                                <form action="{{ route('frontend.test-results.destroy', $testResult->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('test_result_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.test-results.massDestroy') }}",
